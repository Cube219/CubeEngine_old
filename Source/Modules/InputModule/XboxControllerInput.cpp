	...
	namespace module
	{
		...
		bool XboxControllerInput::IsButtonPressed(uint32_t playerIndex, DigitalButton button)
		{
			...
			if(playerIndex >= XUSER_MAX_COUNT) {
				CUBE_LOG(LogType::Error, fmt::format(L"Invalid player index (index: {0} / maxIndex: {1})", playerIndex, XUSER_MAX_COUNT - 1));
				return false;
			}
#endif // _DEBUG
			...
			switch(button) {
				...
				default:
					CUBE_LOG(LogType::Error, fmt::format(L"Invalid digital button ({0})", button));
					return false;
			}
		...
		float XboxControllerInput::GetAnalogValue(uint32_t playerIndex, AnalogButton button)
		{
			...
			switch(button) {
				...
				default:
					CUBE_LOG(LogType::Error, fmt::format(L"Invalid analog button ({0})", button));
					return 0.0f;
			}
