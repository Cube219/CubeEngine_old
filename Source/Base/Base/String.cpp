...
namespace cube
{
	...
	U8String ToU8String(const UCS2String& str)
	{
		...
		for(auto iter = str.cbegin(); iter != str.cend(); iter++) {
			u8StrLength += GetUTF8RequiredCharSize(*iter);
		}

		U8String u8Str;
		u8Str.reserve(u8StrLength);
		for(auto iter = str.cbegin(); iter != str.cend(); iter++) {
			InsertCharInUTF8(u8Str, *iter);
		}
	...
	U8String ToU8String(const U16String& str)
	{
		...
		for(auto iter = str.cbegin(); iter != str.cend();) {
			char32_t codePoint = GetUTF16CharAndMove(iter);
			u8StrLength += GetUTF8RequiredCharSize(codePoint);
		}
		...
		for(auto iter = str.cbegin(); iter != str.cend();) {
			char32_t codePoint = GetUTF16CharAndMove(iter);
			InsertCharInUTF8(u8Str, codePoint);
		}
	...
	UCS2String ToUCS2String(const U8String& str)
	{
		...
		for(auto iter = str.cbegin(); iter != str.cend();) {
			...
			int chSize = GetUTF8CharSize(iter);
#endif // _DEBUG
			char32_t ch = GetUTF8CharAndMove(iter);
	...
	UCS2String ToUCS2String(const U16String& str)
	{
		...
		for(auto iter = str.cbegin(); iter != str.cend();) {
			...
			int chSize = GetUTF16CharSize(iter);
#endif // _DEBUG
			char32_t ch = GetUTF16CharAndMove(iter);
	...
	U16String ToU16String(const U32String& str)
	{
		int u16StrLength = 0;
		for(auto iter = str.cbegin(); iter != str.cend(); iter++) {
			u16StrLength += GetUTF16RequiredCharSize(*iter);
		}

		U16String u16Str;
		for(auto iter = str.cbegin(); iter != str.cend(); iter++) {
			InsertCharInUTF16(u16Str, *iter);
		}
