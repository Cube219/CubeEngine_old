...
namespace cube
{
	namespace render
	{
		TextureViewVk::TextureViewVk(DeviceVk& device, const TextureViewAttribute& attr, const VkImageWrapper& image)
		{
			VkResult res;
			...
			// TODO: Deferred rendering때 쓰이지 않을끼?
			...
			if((attr.componentFlags & TextureViewComponentFlagBits::Color) > 0) {
				info.subresourceRange.aspectMask |= VK_IMAGE_ASPECT_COLOR_BIT;
			}
			if((attr.componentFlags & TextureViewComponentFlagBits::Depth) > 0) {
				info.subresourceRange.aspectMask |= VK_IMAGE_ASPECT_DEPTH_BIT;
			}
			if((attr.componentFlags & TextureViewComponentFlagBits::Stencil) > 0) {
				info.subresourceRange.aspectMask |= VK_IMAGE_ASPECT_STENCIL_BIT;
			}

			U8String debugName = fmt::format("{0} (Texture: {1})", attr.debugName, image.mDebugName);
			mImageView = device.GetLogicalDevice()->CreateVkImageViewWrapper(info, debugName.data());
		}
